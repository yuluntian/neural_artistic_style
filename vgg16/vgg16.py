...
#                                                                                      #
# Original VGG16 model from Davi Frossard                                              #                                                   
...
class vgg16:
    def __init__(self, img=None, train=False):
        self.train = train
        self.parameters = [] # used for loading pre-trained model
        self.inputlayer(img, self.train)
        self.convlayers()
    ...
    def convlayers(self):
        ...
        self.pool5 = tf.nn.max_pool(self.conv5_3,
                               ...
                               name='pool4')
    def adjust(self):
        self.img = tf.maximum(self.img, tf.zeros(tf.shape(self.img), tf.float32))
        self.img = tf.minimum(self.img, 255.0 * tf.ones(tf.shape(self.img), tf.float32))

    def content_response(self):
        return self.conv4_2

    def style_response(self):
        layers = [self.conv1_1, self.conv2_1, self.conv3_1, self.conv4_1, self.conv5_1]
        grams = []
        for layer in layers:
        ...
        # return the concatenation of all gram vector
    ...
    # Load pretrained weights from file, skipping fully connected layers since they are not used.
    def load_weights(self, weight_file, sess, skip_keys=['fc6_W', 'fc6_b', 'fc7_W', 'fc7_b', 'fc8_W', 'fc8_b']):
